...
def truncate_seq_triple(tokens_a, tokens_b, tokens_c, max_num_tokens):
    """ truncate sequence pair to specific length """
    while True:
        ...
        trunc_tokens = tokens_a if len(tokens_a) > len(tokens_b) else tokens_b
        trunc_tokens = trunc_tokens if len(trunc_tokens) > len(tokens_c) else tokens_c
            ...
            while True:
                ...
                if not line.strip(): # 空行
                    ...
                    if len(docs_buffer) == self.docs_buffer_size:
                        ...
                        instances = self.build_instances(docs_buffer)
                        # Save instances.
                        # 数据内容为（src, tgt_mlm, is_random_next, seg_pos）
                        # src: mask后的句子，tgt_mlm: mask的位置和原值，is_random_next: 是否随机设置下一个句子，seg_pos: 分隔符位置
                        for instance in instances: 
                            pickle.dump(instance, dataset_writer)
                        # Clear buffer.
                        docs_buffer = []
                    continue
                sentence = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(line)) # 根据tokenizer将句子转换为id
                if len(sentence) > 0:
        ...
        while i < len(document):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    else: # 只有当当前的document有多个短(<125)句子时才会执行这个分支
                        is_random_next = 0
                    ...
                    while len(src) != self.seq_length: # 补全到seq_length
                        src.append(PAD_ID)
                    ...
                    if not self.dynamic_masking:
                        ...
                        instance = (src, tgt_mlm, is_random_next, seg_pos) # mask后的句子，mask的位置和原值，是否是下一个句子，分隔符位置
                    else:
...
class BertV1Dataset(Dataset):
    """
    滑动窗口取三个连续的数据包，cls预测中间的数据包是否是连续数据包的中间包
    """

    def __init__(self, args, vocab, tokenizer):
        super(BertV1Dataset, self).__init__(args, vocab, tokenizer)
        self.short_seq_prob = args.short_seq_prob
    ...
    def worker(self, proc_id, start, end):
        ...
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            ...
            while True:
                ...
                if pos >= end: # 到达结束位置
                    if len(docs_buffer) > 0:
                        instances = self.build_instances(docs_buffer, docs_timestamp_buffer)
                        for instance in instances:
                            pickle.dump(instance, dataset_writer)
                    break
                ...
                if not line.strip(): # 空行
                    if len(document) >= 1:
                        docs_buffer.append(document)
                        docs_timestamp_buffer.append(document_timestamp)
                    document = []
                    document_timestamp = []
                    if len(docs_buffer) == self.docs_buffer_size:
                        # Build instances from documents.
    ...
    def build_instances(self, all_documents, all_doc_timestamps):
        instances = []
        for _ in range(self.dup_factor):
            for doc_index in range(len(all_documents)):
                instances.extend(self.create_ins_from_doc(all_documents, doc_index, all_doc_timestamps))
        return instances

    def create_ins_from_doc(self, all_documents, document_index, all_doc_timestamps):
        document = all_documents[document_index]
        max_num_tokens = self.seq_length - 4
        target_seq_length = max_num_tokens
        if random.random() < self.short_seq_prob: # 为什么要随机设置短句子的概率
            target_seq_length = random.randint(3, max_num_tokens) # 2->3
        instances = []

        # 计算time pos int(all_doc_timestamps[0][1] * 1e3 % 1e6) - int(all_doc_timestamps[0][0] * 1e3 % 1e6)
        time_pos_a = 0
        time_pos_b = int(all_doc_timestamps[document_index][1] * 1e3 % 1e6) - int(all_doc_timestamps[document_index][0] * 1e3 % 1e6)
        time_pos_c = int(all_doc_timestamps[document_index][2] * 1e3 % 1e6) - int(all_doc_timestamps[document_index][0] * 1e3 % 1e6)
        ...
        if random.random() < 0.5:
            ...
            random_document = all_documents[random_document_index]
            random_segment_index = random.randint(1,2)
            tokens_b.extend(random_document[random_segment_index])
            time_pos_b = int(all_doc_timestamps[random_document_index][random_segment_index] * 1e3 % 1e6) - int(all_doc_timestamps[random_document_index][random_segment_index - 1] * 1e3 % 1e6)
        else:
            tokens_b.extend(document[1])
        
        tokens_c = []
        tokens_c.extend(document[2])

        time_pos = [time_pos_a, time_pos_b, time_pos_c]

        truncate_seq_triple(tokens_a, tokens_b, tokens_c, max_num_tokens)
        ...
        # assert len(src) <= self.seq_length
        while len(src) != self.seq_length: # 补全到seq_length
            src.append(PAD_ID)

        if not self.dynamic_masking:
            src, tgt_mlm = mask_seq(src, self.tokenizer, self.whole_word_masking, self.span_masking, self.span_geo_prob, self.span_max_length)
            instance = (src, tgt_mlm, is_random_next, seg_pos, time_pos) # mask后的句子，mask的位置和原值，是否是下一个句子，分隔符位置
        else:
            instance = (src, is_random_next, seg_pos, time_pos)
...
class BertV1DataLoader(DataLoader):
    def __iter__(self):
        while True:
            while self._empty():
                self._fill_buf()
            ...
            for ins in instances: # ins: (src, tgt_mlm, is_random_next, seg_pos, time_pos)
                if len(ins) == 5:
                    src.append(ins[0])
                    masked_words_num += len(ins[1])
                    tgt_mlm.append([0] * len(ins[0]))
                    for mask in ins[1]:
                        tgt_mlm[-1][mask[0]] = mask[1]
                    is_next.append(ins[2]) # 这里的is_next是是否是随机下一个句子，后面计算的时候是不是算反了 TODO
                    # seg.append([1] * ins[3][0] + [2] * (ins[3][1] - ins[3][0]) + [PAD_ID] * (len(ins[0]) - ins[3][1]))
                    seg.append([1] * ins[3][0] + [1 + ins[4][1]] * (ins[3][1] - ins[3][0]) + [1 + ins[4][2]] * (ins[3][2] - ins[3][1]) + [PAD_ID] * (len(ins[0]) - ins[3][2]))
                else:
                    src_single, tgt_mlm_single = mask_seq(ins[0], self.tokenizer, self.whole_word_masking, self.span_masking, self.span_geo_prob, self.span_max_length)
                    masked_words_num += len(tgt_mlm_single)
                    src.append(src_single)
