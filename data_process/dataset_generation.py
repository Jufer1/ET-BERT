...
word_dir = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\data_process\\corporayxy\\"
...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level = 'flow'):
    ...
    if dataset_level == 'flow':
        cmd = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\yxyTool\\SplitCap.exe -r %s -s session -o " + output_path
    elif dataset_level == 'packet':
        cmd = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\yxyTool\\SplitCap.exe -r %s -s packets 1 -o " + output_path
...
def get_burst_feature(label_pcap, payload_len):
    ...
    if len(packet_direction) == len(packets):
        ...
        for packet_index in range(len(packets)):
            ...
            packet_string = data.decode()[:2*payload_len] # 这里是截取前128字节的数据包内容
...
def pretrain_dataset_generation(pcap_path):
    ...
    output_split_path = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\data_process\\datasetyxy\\OutputSplitPath\\"
    pcap_output_path = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\data_process\\datasetyxy\\pcapOutputPath\\"
...
if __name__ == '__main__':
    # pretrain
    # pcap_path = "I:\\pcaps\\"
    pcap_path = "D:\\desktop\\workcache\\aiLabSpace\\ET-BERT\\datasets\\Supplementary_Dataset_from_completePCAPs\\"
    # tls 13 downstream
